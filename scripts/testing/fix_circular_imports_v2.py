#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
自动修复循环导入 V2 - 正确处理导入名称设置为 None
"""

import sys
from pathlib import Path
import re
import ast

def safe_print(msg):
    try:
        print(msg)
    except OSError:
        pass

def extract_imported_names(import_lines):
    """从导入语句中提取所有导入的名称"""
    # 合并多行导入为一个完整语句
    full_import = ' '.join(line.strip() for line in import_lines)

    names = []
    try:
        # 使用 AST 解析导入语句
        tree = ast.parse(full_import)
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                for alias in node.names:
                    if alias.name == '*':
                        # 跳过 from x import * 这种情况
                        continue
                    # 使用 as 别名（如果有）或原始名称
                    name = alias.asname if alias.asname else alias.name
                    names.append(name)
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    # 使用 as 别名（如果有）或原始名称
                    name = alias.asname if alias.asname else alias.name
                    names.append(name)
    except SyntaxError:
        # 如果AST解析失败，使用正则表达式回退
        safe_print(f"  ⚠️  AST解析失败，使用正则表达式: {full_import[:50]}")
        # from xxx import a, b, c
        match = re.search(r'from\s+[\w.]+\s+import\s+(.+)', full_import)
        if match:
            imports_part = match.group(1)
            # 移除括号
            imports_part = imports_part.replace('(', '').replace(')', '')
            # 分割名称
            for part in imports_part.split(','):
                part = part.strip()
                if ' as ' in part:
                    # 处理 import X as Y
                    name = part.split(' as ')[1].strip()
                else:
                    name = part.strip()
                if name and name != '*':
                    names.append(name)
        else:
            # import xxx
            match = re.search(r'import\s+([\w.]+)(?:\s+as\s+([\w.]+))?', full_import)
            if match:
                name = match.group(2) if match.group(2) else match.group(1)
                names.append(name)

    return names

def fix_init_file(init_file: Path, dry_run: bool = True) -> bool:
    """为 __init__.py 文件添加 try-except 保护，正确设置 None"""
    try:
        with open(init_file, 'r', encoding='utf-8') as f:
            content = f.read()

        lines = content.split('\n')
        new_lines = []
        i = 0

        while i < len(lines):
            line = lines[i]
            stripped = line.strip()

            # 检查是否是 try: 语句，且下一行是 apt 导入
            if stripped == 'try:':
                indent = len(line) - len(line.lstrip())
                indent_str = ' ' * indent

                # 检查下一行是否是 apt 导入
                if i + 1 < len(lines):
                    next_line = lines[i + 1]
                    next_stripped = next_line.strip()

                    if next_stripped.startswith('from apt.') or next_stripped.startswith('import apt.'):
                        # 找到 try-except 块
                        # 收集整个 try-except 块
                        try_lines = [line]
                        import_lines = []
                        j = i + 1
                        found_except = False
                        except_line_idx = -1

                        # 收集 try 块中的导入语句
                        while j < len(lines):
                            curr_line = lines[j]
                            curr_stripped = curr_line.strip()

                            if curr_stripped.startswith('except ImportError:'):
                                found_except = True
                                except_line_idx = j
                                break
                            elif curr_stripped.startswith('from apt.') or curr_stripped.startswith('import apt.'):
                                # 导入语句
                                import_start = j
                                import_block = [curr_line]
                                j += 1

                                # 收集多行导入
                                while j < len(lines):
                                    imp_next = lines[j]
                                    imp_next_stripped = imp_next.strip()

                                    if imp_next_stripped and (
                                        imp_next.startswith(indent_str + '        ') or
                                        imp_next_stripped.startswith(')') or
                                        (import_block[-1].rstrip().endswith('(') or
                                         import_block[-1].rstrip().endswith(','))
                                    ):
                                        import_block.append(imp_next)
                                        j += 1
                                    else:
                                        break

                                import_lines.extend(import_block)
                            else:
                                j += 1

                        # 检查 except 块是否只有 pass
                        if found_except and import_lines:
                            # 检查 except 后面是否只有 pass
                            has_only_pass = False
                            if except_line_idx + 1 < len(lines):
                                except_body = lines[except_line_idx + 1].strip()
                                if except_body == 'pass':
                                    has_only_pass = True

                            if has_only_pass:
                                # 需要修复：提取导入的名称
                                imported_names = extract_imported_names(import_lines)

                                # 生成正确的 try-except 块
                                new_lines.append(f"{indent_str}try:")
                                for imp_line in import_lines:
                                    new_lines.append(imp_line)
                                new_lines.append(f"{indent_str}except ImportError:")

                                # 为每个导入的名称设置为 None
                                for name in imported_names:
                                    new_lines.append(f"{indent_str}    {name} = None")

                                # 跳过原来的 try-except-pass 块
                                i = except_line_idx + 2  # 跳过 except 和 pass
                                continue

            # 检查是否是未保护的 apt 导入
            if (stripped.startswith('from apt.') or stripped.startswith('import apt.')):
                # 找到未保护的导入
                indent = len(line) - len(line.lstrip())
                indent_str = ' ' * indent

                # 收集连续的导入语句（处理多行导入）
                import_lines = [line]
                j = i + 1

                # 检查是否是多行导入
                while j < len(lines):
                    next_line = lines[j]
                    next_stripped = next_line.strip()

                    # 如果是续行（缩进或括号）
                    if next_stripped and (
                        next_line.startswith(indent_str + '    ') or
                        next_stripped.startswith(')') or
                        (import_lines[-1].rstrip().endswith('(') or
                         import_lines[-1].rstrip().endswith(','))
                    ):
                        import_lines.append(next_line)
                        j += 1
                    else:
                        break

                # 提取导入的名称
                imported_names = extract_imported_names(import_lines)

                # 生成 try-except 包裹的代码
                new_lines.append(f"{indent_str}try:")
                for imp_line in import_lines:
                    new_lines.append(f"    {imp_line}")
                new_lines.append(f"{indent_str}except ImportError:")

                # 为每个导入的名称设置为 None
                for name in imported_names:
                    new_lines.append(f"{indent_str}    {name} = None")

                i = j
                continue

            new_lines.append(line)
            i += 1

        new_content = '\n'.join(new_lines)

        if new_content != content:
            if not dry_run:
                with open(init_file, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                safe_print(f"✓ 已修复: {init_file.relative_to(project_root)}")
            else:
                safe_print(f"需要修复: {init_file.relative_to(project_root)}")
            return True
        else:
            return False

    except Exception as e:
        safe_print(f"✗ 处理失败 {init_file}: {e}")
        import traceback
        traceback.print_exc()
        return False

def main():
    """主函数"""
    global project_root
    project_root = Path('/home/user/APT-Transformer')

    import argparse
    parser = argparse.ArgumentParser(description='自动修复 __init__.py 的循环导入问题 (V2 - 正确设置 None)')
    parser.add_argument('--dry-run', action='store_true', help='仅预览，不实际修改文件')
    parser.add_argument('--apply', action='store_true', help='实际应用修复')
    args = parser.parse_args()

    if not args.apply and not args.dry_run:
        safe_print("请使用 --dry-run 预览或 --apply 应用修复")
        return 1

    dry_run = not args.apply

    safe_print("=" * 70)
    if dry_run:
        safe_print("预览模式 (V2 - 正确设置 None)")
    else:
        safe_print("应用模式 (V2 - 正确设置 None)")
    safe_print("=" * 70)

    apt_dir = project_root / 'apt'
    init_files = list(apt_dir.rglob('__init__.py'))

    safe_print(f"\n找到 {len(init_files)} 个 __init__.py 文件")

    # 不跳过任何文件 - 需要重新修复所有文件
    fixed_count = 0
    for init_file in init_files:
        rel_path = str(init_file.relative_to(project_root))
        if fix_init_file(init_file, dry_run):
            fixed_count += 1

    safe_print("\n" + "=" * 70)
    if dry_run:
        safe_print(f"预览完成: 发现 {fixed_count} 个需要修复的文件")
        safe_print("\n使用 --apply 参数来应用修复:")
        safe_print("  python3 scripts/testing/fix_circular_imports_v2.py --apply")
    else:
        safe_print(f"修复完成: 已修复 {fixed_count} 个文件")
        safe_print("\n建议:")
        safe_print("1. 运行测试确认修复有效")
        safe_print("2. 提交修复: git add apt/ && git commit -m 'fix: Properly set imported names to None in exception handlers'")

    return 0

if __name__ == "__main__":
    sys.exit(main())
