...
测试 apt.model.architectures.claude4_model 的性能表现
"""
import os
import time
import argparse
...
class SimpleClaude4(nn.Module):
    """简化的 Claude 4 风格模型（用于测试）"""
    ...
    def __init__(self, config: Claude4Config):
        ...
        # 使用标准 Transformer（模拟 Claude 4 的结构）
...
def try_import_claude4():
    """尝试导入真实的 Claude 4 模型"""
    try:
        from apt.model.architectures.claude4_model import Claude4Model
        print("✅ 使用真实 Claude 4 模型")
        return True, Claude4Model
    except Exception as e:
        print(f"⚠️ 无法导入真实 Claude 4: {e}")
        print("✅ 使用简化版 Claude 4 风格模型")
        return False, SimpleClaude4
...
def main():
    parser = argparse.ArgumentParser(description="Claude 4 性能测试")
    parser.add_argument("--use-real-claude4", action="store_true",
                       help="使用真实的 Claude 4 模型（如果可用）")
    ...
    # 尝试导入真实模型
    use_real, ModelClass = try_import_claude4()

    # 模型配置
    config = Claude4Config(d_model=768, n_heads=12, n_layers=12, d_ff=3072)

    print(f"\n{'='*60}")
    print(f"Claude 4 架构性能测试")
    print(f"{'='*60}")
    print(f"模型: {'真实 Claude 4' if use_real else '简化版 Claude 4'}")
    ...
    # 创建模型
    if use_real and args.use_real_claude4:
        try:
            model = ModelClass(
