...
def main():
    parser = argparse.ArgumentParser(description="纯 Transformer 性能测试")
    parser.add_argument("--model-name", default="gpt2-small", choices=["gpt2-small", "gpt2-medium"],
                       help="模型大小")
    ...
    if args.model_name == "gpt2-small":
        config = TransformerConfig(d_model=768, n_heads=12, n_layers=12, d_ff=3072)
    else:  # gpt2-medium
        config = TransformerConfig(d_model=1024, n_heads=16, n_layers=24, d_ff=4096)

    print(f"\n{'='*60}")
    print(f"纯 Transformer 性能测试")
    print(f"{'='*60}")
