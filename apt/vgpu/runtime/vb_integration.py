...
if TORCH_AVAILABLE:
    ...
    class VBOptimizedLinear(nn.Module):
        ...
        脉冲策略：95%时间走快速路径(F.linear)，5%时间走脉冲路径(VB量化)
        """
        ...
        def __init__(self, in_features: int, out_features: int,
            ...
            self._fwd_count = 0  # per-layer计数器

        def forward(self, x):
            """前向传播：默认走原生Linear；每N次forward触发一次VB的ShrinkTrace脉冲量化（STE + cache）"""
            # 延迟初始化VB适配器（仅在第一次forward时创建）
            ...
            # 兼容回退：如果adapter还没实现linear_pulse，使用原有compress方法
            W = self.weight
            X = x
