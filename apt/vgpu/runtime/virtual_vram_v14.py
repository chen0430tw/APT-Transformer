...
def virtual_vram(cfg: VirtualVRAMConfig):
    ...
    def _store_in_pages(t: torch.Tensor, tensor_key: Tuple) -> Optional[_Packed]:
        ...
        try:
            ...
            # 计算需要的页数
            num_pages = (nbytes + cfg.page_size - 1) // cfg.page_size
            ...
            # 将数据展平并分页
            flat_data = t.detach().cpu().flatten()

            # 存储到各个页中
            for i, page_id in enumerate(page_ids):
                start_idx = i * cfg.page_size // t.element_size()
                end_idx = min(start_idx + cfg.page_size // t.element_size(), flat_data.numel())

                page_data = flat_data[start_idx:end_idx]

                # 设置物理页数据
                page_allocator.set_page(page_id, page_data)

                # 更新页表映射
                page_table.map(tensor_key, i, page_id)
    ...
    def _load_from_pages(packed: _Packed) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            # 从各个页加载数据
            page_data_list = []
            for i, page_id in enumerate(packed.page_ids):
                page_data = page_allocator.get_page(page_id)
                if page_data is None:
                    if cfg.verbose:
                        print(f"[VirtualVRAM] ⚠️  页 {page_id} 数据丢失")
                    return None
                page_data_list.append(page_data)

            # 拼接数据
            if len(page_data_list) == 1:
                restored_flat = page_data_list[0]
            else:
                restored_flat = torch.cat(page_data_list)

            # 恢复原始形状
            restored = restored_flat.reshape(packed.cpu_tensor.shape)
            restored = restored.to(device=packed.device, non_blocking=False)
            restored.requires_grad_(packed.requires_grad)
