...
| `--nodes=1` | 1个节点 | ✅ 有效 |
| `--nodes=2` | 2个节点 | ✅ 有效 |
| `--nodes=1-2` | 等待2个节点 | ⚠️ 不推荐 |
| `--nodes=1-4` | **自动截断为1-2** | ❌ 超出限制 |
| `--nodes=3` | **提交失败** | ❌ 超出限制 |
| `--nodes=4` | **提交失败** | ❌ 超出限制 |

**重要提示**:
- ❌ **不要使用** `--nodes=1-4`：虽然语法正确，但会被截断为 `--nodes=1-2`
- ❌ **不要使用** `--nodes=3` 或更大值：会因超出分区限制而提交失败
- ✅ **normal2分区**（H200节点）：可能有不同的MaxNodes限制，需查看具体配置
- ✅ **查看分区限制**：`scontrol show partition <partition_name> | grep MaxNodes`
...
**APT项目特点**:
- GPU虚拟化: Virtual VRAM v1.6 + LECaC量化
- 训练平台: Transformer训练，支持分布式
- 测试脚本: 已有WSL Slurm测试环境
- 显存需求: 中等（已在WSL 8GB RTX 3070测试过）

### 测试类型对比

| 测试类型 | 推荐集群 | 原因 |
|---------|---------|------|
| **快速功能验证** (<30分钟) | **TWCC gtest** | 可能免费/优惠，快速调通 |
| **小规模训练** (1-2小时) | **TWCC gp1d** | 低成本，24小时足够 |
| **Virtual VRAM测试** | **WSL本地** | 零成本，快速迭代 |
| **分布式训练验证** | **晶创25** | HDR IB 200Gbps，低延迟 |
| **大模型训练** (>3B参数) | **晶创25** | H100/H200大显存 |
...
```bash
# 已有环境，零成本
cd /mnt/d/apt-transformer
sbatch -p debug slurm/vvram_cluster_test_wsl.sh
```
**优势**:
- ✅ 零成本
- ✅ 快速迭代（几分钟）
...
#### 阶段2: TWCC gtest调通 (0.5小时)
```bash
# 测试环境配置
wsl ssh twcc "cd /work/twsuday816 && sbatch -p gtest test_script.sh"
```
**优势**:
- ✅ 可能免费/优惠
- ✅ 验证集群环境
...
#### 阶段3: TWCC gp1d小规模 (24小时)
```bash
# 小规模训练验证
wsl ssh twcc "cd /work/twsuday816 && sbatch -p gp1d train_script.sh"
```
**优势**:
- ✅ 低成本（约1-3元/SU）
- ✅ 24小时足够小规模训练
...
```bash
# 大规模或时间紧迫
wsl ssh twsuday816@nano5.nchc.org.tw "cd /work/... && sbatch -p normal train_script.sh"
```
**优势**:
- ✅ H100 80GB大显存
- ✅ 高性能，快速完成
...
#### Virtual VRAM v1.6测试
```
推荐: WSL本地 → TWCC gtest → TWCC gp1d

原因:
1. WSL已有完整测试环境
2. Virtual VRAM已在WSL验证
3. TWCC成本低，适合探索性测试
4. 晶创25太贵（120元/GPU小时）
```
...
原因:
1. HDR IB 200Gbps 低延迟（0.03-0.07ms）
2. 多节点性能更好
3. H100/H200算力强
```
...
|------|------|------|------|
| WSL测试 | 本地 | 0元 | 快速迭代 |
| TWCC探索 | TWCC | ~100-500元 | 环境验证+小规模 |
| 晶创25生产 | 晶创25 | >5000元 | 大规模训练 |

**立即可用的最优方案**:
1. WSL本地测试 - 已有环境，零成本
2. TWCC gtest验证 - 可能免费，30分钟调通
3. TWCC gp1d小规模 - 低成本探索
4. 晶创25正式生产 - 大规模/时间紧迫
